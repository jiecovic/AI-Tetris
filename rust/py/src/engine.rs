...
impl TetrisEngine {
    fn build_step_features_dict<'py>(
        ...
        let cur = lock_f.or(prev_f).unwrap_or_default();
        let delta = match (lock_f, prev_f) {
            (Some(p), Some(prev)) => GridDelta {
...
impl TetrisEngine {
    ...
    fn step_action_id_with_features<'py>(
        ...
        let r = self.g.step_action_id(action_id);
        let sf = self.build_step_features_dict(py, prev, after_clear)?;
