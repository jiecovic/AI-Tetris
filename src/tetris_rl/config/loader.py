...
def parse_loaded_config(*, cfg: Dict[str, Any], domain: Optional[ConfigDomain] = None) -> Tuple[ConfigDomain, TrainSpec | DataGenSpec]:
    """
    Parse a raw YAML mapping into a typed spec (TrainSpec or DataGenSpec).

    domain:
      - None: infer from top-level keys (strict; mixed schemas error)
      - "train"/"datagen": force a parser (still strict inside the parser)
    """
    ...
    if dom == "train":
        return "train", parse_train_spec(cfg=root)

    return "datagen", parse_datagen_spec(cfg=root)
...
def load_config(
    ...
    """
    Load a YAML config from disk, parse into a typed spec, and optionally snapshot.
    ...
    dom, spec = parse_loaded_config(cfg=cfg, domain=domain)
    ...
    return LoadedConfig(cfg=cfg, domain=dom, spec=spec, snapshot=snapshot)
